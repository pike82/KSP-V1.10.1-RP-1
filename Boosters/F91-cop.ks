...
local box_RTLS is wndw:addhlayout().
	...
	local RTLSvalue is box_RTLS:ADDTEXTFIELD("8200").// //8200 LEO, 9350 GTO, give a margin of atleast 400m/s
...
local box_RTLSMax is wndw:addhlayout().
	...
	local RTLSMaxvalue is box_RTLSMax:ADDTEXTFIELD("7110").//5.5tonne = 7,110, 11 tonne = 5,850, 17 tonne 4,930 //with 1000 m/s margin for re-entry
...
Global gl_TargetLatLngBoost is ff_GeoConv(8,90,gl_TargetLatLng:lat,gl_TargetLatLng:lng).// short of landing pad for boost back
...
//Gravity turn
	if runmode = 2{
		...
		// MECO Shutdown time
		until ship:mass < 185 or ((sv_Stage1_dV < ASDS_limit) and (ship:mass < 208) and (sv_Stage1_dV < ship:velocity:orbit:mag)){//Start_mass - 362{//(TIME:SECONDS - LOtime) > 148 { //RTLS time Start_mass(570) - 362 = 208
				Wait 0.1. 
...
//Re-entry burn
	if runmode = 4{
		...
		set burn_stp to 34.0. //lowest fuel mass required for a three engine stop and equates to around 400 m/s
		Until (ship:mass < burn_stp) or (ship:airspeed < 600){ //check for max entry burn time, energy, or stop if not enough fuel left to land.
			set vd1 to vecdraw(v(0,0,0), impact:position:vec, green, "Landing", 1.0, true, 0.25).
			set vd2 to vecdraw(v(0,0,0), gl_TargetLatLngSafe:position:vec, blue, "Target", 1.0, true, 0.25).
			wait 0.1.
		}
...
//Landing Glide (no long target safe spot)
	...
	if runmode = 5{
		...
		until ship:Q > 0.01 { //set at 10KPa to ensure enought drag to make controls more responsive
			
			set impact to ground_track(impactData["time"]).
			set headingPID to PIDLOOP(
				0.05, // P: adjust heading 1/20 amounts of the error
				0, // I: don't use error
				0.0, // D: adjust small on rate of change
				-0.1, // min output per loop 1 degree per second
				0.1 //max output per loop 1 degree per second
			).
			...
			set pitchPID to PIDLOOP(
				0.01, // P: adjust pitch 1/10 amounts of the error
				0.0, // I: don't use error
				10, // D: adjust small on rate of change
				-0.5, // min output per loop 2 degree per second
				0.5 //max output per loop 2 degree per second
			).
			set targetpitch to 0. // want slightly long
			...
			set mpitch to mpitch + pitchPID:UPDATE(time:seconds, TimeOnTgt-(impactData["time"]-time:seconds)). //current surface speed compared to target arrival
			set mpitch to min(89,max(mpitch,55)).
			Print "mpitch: " + mpitch.
		...
		Set SteeringManager:PITCHTORQUEADJUST to 100. //negative as we want to reduce the amount
		Set SteeringManager:YAWTORQUEADJUST to 100. //negative as we want to reduce the amount
		Set SteeringManager:ROLLTORQUEADJUST to 1. //negative as we want to reduce the amount
		Set AltstopDist to 1.
		until (trueRadar < stopDist * AltstopDist) and (airspeed < 500) and (trueRadar < 5000 ){ 
			...
			set pitchPID to PIDLOOP(
				0.5, // P: adjust pitch 1/10 amounts of the error
				0.0, // I: don't use error
				1000, // D: adjust small on rate of change
				-0.5, // min output per loop 2 degree per second
				0.5 //max output per loop 2 degree per second
			).
			set targetpitch to 0.//want slightly long
			...
			If ship:mass > 36{
				set AltstopDist to 1.5. //provide extra 50% for engine startup and control on RTLS as have extra fuel
			} Else {
				set AltstopDist to 0.65. //provide less 35% for engine startup with 3 engines to use less fuel
			}
			Print "trueRadar: "+ trueRadar.
			Print "maxDecel: "+ maxDecel.
...
//Landing burn
	if runmode = 6{
		...
		until (0 > trueRadar) or (Ship:Status = "LANDED") or (ship:verticalspeed > 0) {
			...
			If (idealThrottle > 1.2) and ((Ship:Q <0.4) or (faltm["fallTime"] < 5)){
			...
			If (trueRadar > (500 + grndOffset)){
				local dist is hf_geoDistance(ground_track(impactData["time"]),gl_TargetLatLng).
				Print arctan(trueradar/dist).
				//lock ster to getSteering(2.5, gl_TargetLatLng, L_impact, ((ship:altitude+grndOffset)/5)). //AOA, tgt, impact, offset height)//original based on landing
				lock ster to getSteeringTrans(5, gl_TargetLatLng, L_impact, Ship:Q/0.15, ((ship:altitude+grndOffset)/10)). //AOA, tgt, impact, ratio, offset height) //bigger ratio more atmosphere
				Set throt_lim to (idealThrottle*0.9).
				Print "Lift major".
			}
			If (trueRadar < (500 + grndOffset)) { // under 500m
				//lock ster to getSteering(7.5, gl_TargetLatLng, L_impact, ((ship:altitude+grndOffset)/10)). //AOA, tgt, impact, offset height)
				lock ster to getSteeringTrans(10, gl_TargetLatLng, L_impact, Ship:Q/0.15, ((ship:altitude+grndOffset)/10)). //AOA, tgt, impact, ratio, offset height)
				Set throt_lim to (idealThrottle*0.99).
				Print "Lift medium".
			}
			If ship:Q < 0.01{ // 10.0kpa narrow AOA and make engine prime steering (was 12.5kpa)
				lock ster to getSteeringEngine(15, gl_TargetLatLng, L_impact,((ship:altitude+grndOffset)/15)). //AOA, tgt, impact, offest height. move to ship position instead of impact//15
			...
			If airspeed < 50 { // 50 m/s narrow AOA and keep engine prime
				lock ster to getSteeringEngine(20, gl_TargetLatLng, L_impact). //AOA, tgt, impact, offest height. move to ship position instead of impact
				Print "Engine 50".
			}
			...
			if (trueRadar < (200 + grndOffset)) { //cancel out ground speed
				lock steering to getSteeringEngineStop (20).
				Print "Locked retro".
			}
