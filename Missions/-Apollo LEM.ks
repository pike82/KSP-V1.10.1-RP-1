...
If runmode = 1.0{
	...
	Print gl_LandTime.
	local startTime is (gl_LandTime - (p_time["profiletime"]/2)).
	//local startTime is (gl_LandTime - (p_time["profiletime"]*2)).
	...
	//Start main engines
	Lock Throttle to 1.
	ff_ProfileLand(0.9, 250, p_time["profiletime"]).
...
If runmode = 4.0{
	unlock all.
	Set SHIP:CONTROL:PILOTMAINTHROTTLE TO 0.
	local tgt_az is ff_FlightAzimuth(target:orbit:inclination, 1650).
	ff_Avionics_on().
	RCS on.
	Stage.
	lock throttle to 1.
...
If runmode = 5.0{
	...
	Add n.
	local startTime is time:seconds + nextnode:eta - (ff_Burn_Time(nextnode:deltaV:mag / 2, 311, 15.5, 1)).
	lock steering to nextnode:burnvector.
...
Parameter lastdt, lastPos.
	...
	Set LatDist to hf_geoDistance(LATLNG(tgt:Lng,gl_shipLatLng:Lat),tgt). // distance to tgt in Lat metres only
	...
	Set sv_PIDNorth to PIDLOOP(10000, 0, 0, -5, 5).//SET PID TO PIDLOOP(KP, KI, KD, MINOUTPUT, MAXOUTPUT).
	Set sv_PIDNorth:SETPOINT to LATSpeed. // the new desired speed in m/s
	Set sv_PIDEast to PIDLOOP(10000, 0, 0, -5, 5).//SET PID TO PIDLOOP(KP, KI, KD, MINOUTPUT, MAXOUTPUT).
	Set sv_PIDEast:SETPOINT to LONGSpeed. // the new desired speed in m/s
	
	Set NorthSpeed to abs(gl_shipLatLng:Lat - LastPos:Lat)/(TIME:SECONDS-lastdt).
	Set EastSpeed to abs(gl_shipLatLng:Lng - LastPos:Lng)/(TIME:SECONDS-lastdt).
	...
	//SET NorthDirection TO sv_PIDNorth:UPDATE(TIME:SECONDS, NorthSpeed1). // PID the North velocity with the new desired speed
	//SET EastDirection TO sv_PIDEast:UPDATE(TIME:SECONDS, EastSpeed1). // PID the East velocity with the new desired speed
	...
	Set SteerDirection to UP + r(NorthDirection,EastDirection,90). // r(pitch, yaw, roll) set roll to zero, this will allow pitch to equal Lat(North) direction required and Yaw(East) to equal Long direction required		
	Set SteerDirection to HEADING(tgt:heading,90 - sqrt((NorthDirection^2)+(EastDirection^2))). // inital setup HEADING(degreesFromNorth, pitchAboveHorizon).
...
Function ff_ProfileLand{ 
	...
	Set PIDVV to PIDLOOP(0.05, 0, 0.01, -0.06, 0.06).//SET PID TO PIDLOOP(KP, KI, KD, MINOUTPUT, MAXOUTPUT).	
	Set PIDVV:SETPOINT to 0. // we want the altitude to remain constant so no vertical velocity.
	Set highpitch to 0.
	
	Set PIDFALL to PIDLOOP((ship:availablethrust / (ship:mass*100)), 0, 0.1, -50, 0).//SET PID TO PIDLOOP(KP, KI, KD, MINOUTPUT, MAXOUTPUT).	
	...
	until (((Basetime + (profiletime*1.1) ) - time:seconds) < 0) OR (SHIP:GROUNDSPEED < 2){ // Unil we should have stopped based on time or we have actually stopped TODO: Make it so the +10 is not hardcoded in.
		...
		Set FALLSpeed to PIDFALL:UPDATE(TIME:SECONDS, ((ALT:RADAR - tgtFallheight)/2)). //set the allowable fall speed to ensure we don't fall so fast we will crash
...
Parameter Hover_alt is 50, BaseLoc is SHIP:GEOPOSITION. 
	...
	Until distanceTol > 30 or (ALT:RADAR < 1) or (Ship:Status = "LANDED") { // until the ship is hovering above the set down loaction for 3 seconds (to allow for PID stability before heading down)
		ClearScreen.
		Print "Moving to Landing Position".	
		Set dtStore to hf_LandingPIDControlLoop(dtStore["lastdt"], dtStore["lastPos"]).
		if hf_geoDistance(BaseLoc, SHIP:GEOPOSITION) < 30{ //30m error before descent. Using geoDistance as distance is expected to be small (less than 1km)
